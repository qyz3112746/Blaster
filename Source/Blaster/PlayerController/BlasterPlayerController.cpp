...
void ABlasterPlayerController::CheckTimeSync(float DeltaTime)
{
	...
	if (IsLocalController() && TimeSyncRunningTime >= TimeSuncFerquency)
	{
...
void ABlasterPlayerController::SetHUDWeaponAmmo(int32 Ammo)
{
	...
	if (bHUDValid)
	{
		...
		FSlateColor TextColor = Ammo == 0 ? FSlateColor(FLinearColor::Red) : FSlateColor(FLinearColor::White);
		BlasterHUD->CharacterOverlay->WeaponAmmoAmount->SetColorAndOpacity(TextColor);
	}
	else
	{
	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				if (bInitializeRedTeamScore) SetHUDRedTeamScore(HUDRedTeamScore);
				if (bInitializeBlueTeamScore) SetHUDBlueTeamScore(HUDBlueTeamScore);
...
void ABlasterPlayerController::HideTeamScores()
{
	...
	if (bHUDValid)
	{
		if (BlasterHUD->CharacterOverlay->BlueTeamScore)
		{
			BlasterHUD->CharacterOverlay->BlueTeamScore->SetText(FText());
		}
		if (BlasterHUD->CharacterOverlay->RedTeamScore)
		{
...
{
	SetHUDBlueTeamScore(0.f);
	SetHUDRedTeamScore(0.f);
}
